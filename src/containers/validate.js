...
export const registerValidate = values => {
  ...
  if (
    values.password &&
    !/^(?=.*[äöåa-z])(?=.*[ÄÖÅA-Z])(?=.*\d)[äöåa-zÄÖÅA-Z\d]*\S{8,}$/.test(
      values.password
    )
  ) {
...
export const jobPostFormValidate = values => {
  ...
  if (
    (values.due_date &&
      ...
      new Date(values.due_date).setHours(0, 0, 0, 0) -
    ...
    (values.due_date &&
      new Date(values.due_date).getTime() >
...
export const profileValidate = values => {
  ...
  if (values.companyName && isEmpty(values.companyName))
    errors["companyName"] = i18next.t("validation:emptyStr");

  if (values.address && isEmpty(values.address))
    errors["address"] = i18next.t("validation:emptyStr");

  // zip code validation
  if (values.zipCode && !/^\d{5}$/i.test(values.zipCode)) {
    errors.zipCode = i18next.t("validation:postInvalid");
  }

  // business_id validation
  if (
    values.companyBusinessId &&
    !/^\d{7}-\d{1}$/i.test(values.companyBusinessId)
  ) {
    errors.companyBusinessId = i18next.t("validation:businessIdInvalid");
  }

  // phoneNumber validation
  if (values.telephone && !/^\+?[0-9 ]{7,13}$/i.test(values.telephone)) {
    errors.telephone = i18next.t("validation:phoneInvalid");
  }
  ...
  if (
    ...
    errors.companyUrl = i18next.t("validation:urlInvalid");
  }
    ...
    values.companyUser.forEach((user, i) => {
      ...
      if (
        ...
        (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(
          ...
          values.companyUser[i].email === values.email)
      ) {
...
export const jobseekerProfileValidate = values => {
  ...
  if (values.profileDescription && isEmpty(values.profileDescription)) {
    errors["profileDescription"] = i18next.t("validation:emptyStr");
  }
  return errors;
};
...
export const marketingDetailsValidate = values => {
  ...
  if (
    values.marketing_budget &&
    (parseInt(values.marketing_budget) < 1 ||
      !/^[0-9]+$/.test(values.marketing_budget))
  ) {
...
export const interviewDetailsValidate = values => {
  ...
  if (
    values.interview_title &&
    isMaxLengthExceeded(values.interview_title, 100)
  ) {
  ...
  if (
    !values.interview_msg ||
    (values.interview_msg && isRteEmpty(values.interview_msg))
  ) {
    errors.interview_msg = i18next.t("validation:fieldRequired&maxLength");
  }
  if (
    values.interview_place &&
    isMaxLengthExceeded(values.interview_place, 100)
  ) {
...
export const applicationFormValidate = values => {
  ...
  if (
    ...
    !/^\+?[0-9 ]{7,13}$/i.test(values.contact_number)
  ) {
    errors.contact_number = i18next.t("validation:phoneInvalid");
  }

  if (
    values.application_description &&
    isMaxLengthExceeded(values.application_description, 7000)
  ) {
    errors.application_description = i18next.t(
      "validation:maxLengthApplicationDesc"
    );
  }
...
export const usersProfileValidate = values => {
  ...
  if (values.companyBusinessId) {
    requiredFields = ["companyName", "firstName", "lastName", "email"];
  } else {
  ...
  if (
    values.companyUrl &&
    !/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_.~#()?&//=]*)/gim.test(
      values.companyUrl
    )
  ) {
    errors.companyUrl = i18next.t("validation:urlInvalid");
