...
const mapStateToProps = (state) => {
  const userRole =
    state.client.user &&
    state.client.user.data &&
    state.client.user.data.user_type;
  return {
    state,
    ...
    advertPages: Math.ceil(
      state.jobs.jobsList[0]?.full_count / JOBPOST_COUNT_PER_PAGE
    ),
    showSpinner: state.jobs.showSpinner,
    showSuccessSnackbar: state.asyncActions.showSuccessSnackbar,
    favoriteJobs: state.jobs.dashboard.favoriteJobs,
    showDialog:
      userRole === "admin"
        ? state.advertisement.warnToDelete
        : state.asyncActions.showDialog, // For admins, we shouw the delete dialog
    userRole,
